...
void PrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent){
  ...
  for(int l=0;l<fnlevel;l++){
	...
	fenergy[l] = RandFlat::shoot(fset->GammaWhiteFrom(),fset->GammaWhiteTo());
      ...
      double theta = fset->GammaDirectionTheta()*deg2rad;
      double phi = fset->GammaDirectionPhi()*deg2rad;
	...
	// create a vector
	v.set(0.0,0.0,1.0);
	theta = fset->GammaDirectionTheta()*rad2deg;
	phi = fset->GammaDirectionPhi()*rad2deg;
	if(fset->GammaDirection()!=2){
	  // random cos(theta)  = -1..1
	  theta = RandFlat::shoot(-1,1);
	  // theta = random in 0..180 degree
